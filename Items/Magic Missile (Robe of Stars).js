...
if (lastArg.targets.length === 1) {
    ...
    totalDamage = new Roll(`${missiles} * ${damageRoll.total}`).evaluate({ async: false });
    await anime(tokenD, target, missiles);
    ...
    damage_result.push(`<div style="text-align:center">(${CONFIG.DND5E.damageTypes[damageType]})</div><div class="dice-roll"><div class="dice-result"><div class="dice-formula">${damageRoll.formula} x ${missiles} </div><div class="dice-tooltip"><section class="tooltip-part"><div class="dice"><header class="part-header flexrow"><span class="part-formula">${damageRoll.formula}</span><span class="part-total">${damageRoll.total}</span></header><ol class="dice-rolls"><li class="roll die d${damageRoll.dice[0].faces}">${damageRoll.dice[0].total}</li></ol></div></section></div><h4 class="dice-total">${totalDamage.total}</h4></div></div>`);
    await hitList();
    await rollList();
} else if (lastArg.targets.length > 1) {
    let targetList = lastArg.targets.reduce((list, target) => { return list + `<tr><td>${target.name}</td><td><input type="num" id="target" min="0" max="${missiles}" name="${target.id}"></td></tr>` }, "");
    let the_content = `<p>You have currently <b>${missiles}</b> total ${itemD.name} bolts.</p><form class="flexcol"><table width="100%"><tbody><tr><th>Target</th><th>Number Bolts</th></tr>${targetList}</tbody></table></form>`;
