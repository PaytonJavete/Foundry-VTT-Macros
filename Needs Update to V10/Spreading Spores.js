...
async function rollItemDamage(targetToken, itemUuid, itemLevel) {
  ...
  let save_roll = await targetActor.rollAbilitySave('con', {chatMessage : true, async: true });
  if (save_roll.total >= 18){
    let save_result = `${targetToken.name} successfully made their constitution saving throw`;
    ChatMessage.create({
        user: game.user._id,
