...
async function rollItemDamage(targetToken, itemUuid, itemLevel) {
  ...
  let save_roll = await targetActor.rollAbilitySave(saveAbility, {chatMessage : true, async: true });
...
async function attachSequencerFileToTemplate(templateUuid, originUuid) {
  if (game.modules.get("sequencer")?.active) {
    ...
    new Sequence()
    .effect()
      .file("modules/JB2A_DnD5e/Library/2nd_Level/Flaming_Sphere/FlamingSphere_01_Orange_200x200.webm")
      .size({
...
if (args[0].tag === "OnUse" && args[0].macroPass === "preActiveEffects") {
  ...
  await template.update({"flags":{"perfect-vision":{"sightLimit":0}}}); 
